...
#Data frame dos parâmetros e seu respectivo MAE
resultados <- data.frame(matrix(data=NA,nrow=nrow(max_grid),ncol=8))
colnames(resultados) <- c(colnames(max_grid),"MAE")
...
for(linha in 1:nrow(resultados)){
  cat("Testando combinação de parâmetros:", linha, "/", nrow(max_grid), "\n")
  
  #Hiperparâmetros a ser testado
  params <- max_grid[linha,]
  
  #Preenchendo o resultados com os parâmetros a ser testado
  resultados[linha,1:7] <- params
  
  precisao <- funcXGBOOST(dados.proposto,params,folds[[1]])
  print(precisao)
  
  # Armazenar a média dos resultados dos folds no data frame de resultados
  resultados[linha,8] <- precisao
